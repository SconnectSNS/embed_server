...
def embed():
    ...
    max_length = max(
        tokenizer(keyword, truncation=True, return_tensors="pt")['input_ids'].shape[-1] for keyword in keywords)

    inputs = []
    # 기존 DB에 존재하던 게시글들의 토큰을 구합니다.
    for keyword in keywords:
        inputs.append(
            tokenizer(keyword, padding='max_length', truncation=True, max_length=max_length, return_tensors="pt"))
    ...
    for i in range(len(keywords)):
        results[i + 1] = [k[0] for k in similarity_matrix[i][:5]]
